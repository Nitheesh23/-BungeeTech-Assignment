...
function Banner() {
  ...
  useEffect(() => {
    ...
    axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=1749ee86927c862e6ac40360e3eb8c0d&language=en-US&page=1`)
      .then(res => {
  ...
  return (
    <div className='h-[20vh] md:h-[75vh] bg-cover flex items-end' style={{ backgroundImage: `url(${bannerImage})` }}>
      <div className='text-white w-full text-center text-2xl md:text-4xl'>{title}</div>
    </div>
  );
}
